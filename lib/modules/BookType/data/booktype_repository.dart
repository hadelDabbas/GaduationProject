...
class BookTypeRepository implements IBookTypeRepository{
  ...
  Future<List<BookType>> GetAllBookType() async {
      var result =
        await _dio.get('https://localhost:7192/api/BookType');
    var list = <BookType>[];
  ...
  Future<bool> AddBookType(BookType book)async  {
    var result = await _dio.post('https://localhost:7192/api/BookType',
        data: book.toJson());
    return result.statusCode == 200;
  }
  
  @override
  Future<bool> DelBookType(int id) async {
 var result = await _dio.delete(
      'https://localhost:7192/api/BookType/$id',
    );
    return result.statusCode == 200;
  }}