...
import 'package:graduationproject/modules/genereted/sheard/auth_service.dart';
...
var id=auth.getDataFromStorage()!.Id;
  var result=  await _dio.get('https://localhost:7252/api/Main/Posts',queryParameters: {'IdUser':id});
  ...
  Future<bool> AddComment(Comments comments, int Iduser)async {
    var data=  await _dio.post('https://localhost:7252/api/Post',data:comments.toJson());
 ...
 var data= await _dio.put('https://localhost:7252/api/UserPost/$idpost',data: userPost.toJson());
     if (ExpastionStatus(data.statusCode!)) {
      return true;
    } else
      print(data.statusMessage);
